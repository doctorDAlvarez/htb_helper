...
# Fuzz for reaction to special chars (add FUZZ to spots to fuzz in request)
...
#raft-[ small | medium | large ]-files.txt files from SecLists.
#raft-[ small | medium | large ]-directories.txt files from SecLists.
#raft-[ small | medium | large ]-words.txt files from SecLists.
...
#have a list of common backup files or unreference extensions
